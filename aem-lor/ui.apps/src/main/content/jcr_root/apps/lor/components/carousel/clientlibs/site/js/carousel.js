...
const hero = {
	...
	init: function() {
        var isProjects = $(".js-slider").hasClass("projects");
	...
	automaticSlider: function(item, ind) {
        /*if (false) {
			...
			const $slidesProjects = $(item).find('.projects__slide');
            const $slides = $(item).find('.cmp-carousel__item');

            const $currentSlideProjects = $slidesProjects.eq(ind);
            const $currentItemSlide = $slides.eq(ind);
            const $currentSlide = $slides.eq(ind).find('.hero__slide');
            //console.log($currentSlide)
            //const $currentSlide = $slides.eq(ind);
            const $currnetVideo = $currentSlide.find('video');
            const $currentIframe = $currentSlide.find('iframe');
            const currentSlideNum = $(item).find('.js-slider-current');
            //const currentSlideNumProjects = $(item).find('.js-slider-current-projects');
            if ($currnetVideo.length) {
                $currnetVideo[0].currentTime = 0;
            }
            if ($currentIframe.length) {
            ...
            $slidesProjects.not($currentSlideProjects).removeClass(this.activeClass);
            $currentSlideProjects.addClass(this.activeClass);

            currentSlideNum.html(ind + 1);
            //currentSlideNumProjects.html(ind + 1);
